...
static void CreateVertexBuffer(){
	// Vector3f Vertices[1]; // Struct with x,y,z
	// Vertices[0] = Vector3f(0.0f, 0.0f, 0.0f);
	float Vertices [3] {0.0f, 0.0f, 0.0f};
	glGenBuffers(1, &VBO);
	glBindBuffer(GL_ARRAY_BUFFER,VBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float)*3, Vertices, GL_STATIC_DRAW); // the third attribute is a pointer, 
}
...
int main(int argc, char** argv){
	...
	GLclampf Red = 1.0f, Blue = 0.0f, Green = 0.0, Alpha = 0.0f;
