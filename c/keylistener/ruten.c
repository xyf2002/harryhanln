...
void enableRAWMode()
{
	...
	atexit(&disableRAWMode);  // From <stdlib.h> Execute the function when the
							  // program exits.
	InitOriginal = 1;
	raw.c_lflag &= ~(ECHO | ICANON);  // Turn off echo mode. IE, output will not
									  // be displayed while typing.
...
void disableRAWMode()
{
	if (InitOriginal)  // It is impossible to check if the struct is initialized
					   // or not by itself.
...
void *asciRead()
{
	...
	while ((ch = getchar()) != 27) {
		printf("%c\n", ch);
		// usleep(1); // Sleeps a millisecond so that the thread is not taking
		// all the cpu
	}
	ESCPressed = 1;
	printf("%s\n", "ESC Pressed!");
